...
<template>
    <div class="max-w-2xl mx-auto m-3">
        ...
        <div class="m-4">
            ...
            <select v-model="form.activity_type" class="w-full p-2 border rounded mt-2 text-black">
            ...
            <div v-if="form.activity_type === 'feeding'" class="mt-4">
                <label class="block">Typ kŕmenia:</label>
                <select v-model="form.type" class="w-full p-2 border rounded text-black">
                    <option value="breast">Dojčenie</option>
                    <option value="formula">Umelé mlieko</option>
                </select>

                <div v-if="form.type === 'breast'" class="mt-2">
                    <label class="block">Strana prsníka:</label>
                    <select v-model="form.side" class="w-full p-2 border rounded text-black">
                        <option value="left">Ľavý</option>
                        <option value="right">Pravý</option>
                    </select>
                </div>

                <div v-if="form.type === 'formula'" class="mt-2">
                    <label class="block">Množstvo (ml):</label>
                    <input v-model="form.amount" type="number" class="w-full p-2 border rounded text-black" min="0" />
                </div>
            </div>

            <div v-if="form.activity_type === 'diaper_change'" class="mt-4">
