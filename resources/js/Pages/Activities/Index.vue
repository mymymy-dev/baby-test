...
function formatDisplayDate(date) {
    return new Date(date).toLocaleDateString("sk-SK", { day: "2-digit", month: "2-digit" });
}
...
<template>
    <div class="max-w-2xl mx-auto m-3 mb-3 pb-5">
        <div class="flex justify-between items-center">
            ...
            <Link href="/activities/create" class="p-4 me-4 text-white font-semibold">
                ...
                <tbody>
                    <tr v-for="activity in activities" :key="activity.id">
                        <td class="p-2 border-b border-b-gray-800">
                            <div v-if="activity.activity_type.split('\\').pop() === 'Sleep'">🌜</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'Wakeup'">🌞</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'Feeding'">🍼</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'DiaperChange'">💩</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'Bathing'">🛀</div>
                        </td>

                        <td class="p-2 border-b border-b-gray-800">
                            <div v-if="activity.activity_type.split('\\').pop() === 'Wakeup'" class="font-bold">Prebudenie</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'Sleep'" class="font-bold">Spánok</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'Feeding'" class="font-bold">Kŕmenie</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'DiaperChange'" class="font-bold">Prebaľovanie</div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'Bathing'" class="font-bold">Kúpanie</div>

                            <div v-if="activity.activity_type.split('\\').pop() === 'Feeding' && activity.activity">
                                <small v-if="activity.activity.type === 'breast'">Dojčenie</small>
                                <small v-if="activity.activity.type === 'breast' && activity.activity.side === 'left'">, Ľavý prsník</small>
                                <small v-if="activity.activity.type === 'breast' && activity.activity.side === 'right'">, Pravý prsník</small>

                                <small v-if="activity.activity.type === 'formula'">Umelé mlieko</small>
                                <small v-if="activity.activity.type === 'formula' && activity.activity.amount">{{ activity.activity.amount + ' ml' }}</small>
                            </div>
                            <div v-if="activity.activity_type.split('\\').pop() === 'DiaperChange' && activity.activity">
                                <small v-if="activity.activity.type === 'wet' || activity.activity.type === 'both'">Moč</small>
                                <small v-if="activity.activity.type === 'both'">, </small>
                                <small v-if="activity.activity.type === 'dirty' || activity.activity.type === 'both'">Stolica</small>
                            </div>
                        </td>
